...
public class QueryServiceImpl implements QueryService {
    ...
    public void setDefaults(Query query) {
        ...
        if (query.getStartDateOperator() == null)
            query.setStartDateOperator(
                    QueryOptions.getDefaultStartDateOperator());
    ...
    public void validateQuery(Query query) throws InvalidQueryException {
        ...
        try {
            ...
            if (query.getSystemQueryName() == null)
                throw new InvalidQueryException("Missing required parameter systemQueryName: "
                                + query.getSystemQueryName());
            ...
            if (query.getDialogId() != null) {
                ...
                if (!QueryOptions.getDialogIdOptions().contains(query.getDialogId())) {
                    ...
                    throw new InvalidQueryException("Inavlid dialogID: "
                            + query.getDialogId()
                            + ", not one of the supported dialogIDs: "
                            + QueryOptions.getDialogIdOptions().toString());
                }
            ...
            if (query.getStartDateOperator() != null) {
                ...
                if (!QueryOptions.getStartDateOperatorOptions().contains(query.getStartDateOperator())) {
                    throw new InvalidQueryException("Invalid startDateOperator: "
                                    + query.getStartDateOperator()
                                    + ", not one of the supported startDateOperator: "
                                    + QueryOptions.getStartDateOperatorOptions().toString());
                }
            }
            ...
            if (query.getEndDateOperator() != null) {
                ...
                if (!QueryOptions.getEndDateOperatorOptions().contains(query.getEndDateOperator())) {
                    throw new InvalidQueryException("Invalid endDateOperator: "
                            + query.getEndDateOperator()
                            + ", not one of the supported endDateOperator: "
                            + QueryOptions.getEndDateOperatorOptions().toString());
                }
            }

            // OPTIONAL: coordinates
            // If any one coordinate is provided (i.e., not null), all
            // coordinates must be
            // provided.
            if (query.getNwLat() != null || query.getNwLon() != null
                ...
                if (query.getNwLat() == null)
                    throw new InvalidQueryException(
                            "The field nwLat is required");
                if (query.getNwLon() == null)
                    throw new InvalidQueryException(
                            "The field nwLon is required");
                if (query.getSeLat() == null)
                    throw new InvalidQueryException(
                            "The field seLat is required");
                if (query.getSeLon() == null)
                    throw new InvalidQueryException(
                            "The field seLon is required");
            }
            ...
            if (query.getOrderByOrder() != null) {
                ...
                if (!QueryOptions.getOrderByOrderOptions().contains(query.getOrderByOrder())) {
                    throw new InvalidQueryException("Invalid orderByOrder: "
                            + query.getOrderByOrder() + ", must be one of: "
                            + QueryOptions.getOrderByOrderOptions().toString());
                }
            }
            ...
            if (query.getOrderByField() != null) {
                ...
                if (!QueryOptions.getOrderByFieldOptions().contains(query.getOrderByField())) {
                    throw new InvalidQueryException("Invalid orderByField: "
                            + query.getOrderByField() + ", must be one of: "
                            + QueryOptions.getOrderByFieldOptions().toString());
                }
            }
            ...
            if (query.getResultPackaging() != null) {
                ...
                if (!QueryOptions.getResultPackagingOptions().contains(query.getResultPackaging())) {
                    throw new InvalidQueryException("Invalid resultPackaging: "
                            + query.getResultPackaging() + ", must be one of: "
                            + QueryOptions.getResultPackagingOptions().toString());
                }
            }

            // OPTIONAL: skip
            // Skip is parsed to Integer and validated when the query is mapped
            // to the Query object

            // OPTIONAL: limit
            // Limit is parsed to Integer and validated when the query is mapped
            // to the Query object
            ...
            if (query.getResultEncoding() != null) {
                ...
                if (!QueryOptions.getResultEncodingOptions().contains(query.getResultEncoding())) {
                    throw new InvalidQueryException("Invalid resultEncoding: "
                            + query.getResultEncoding()
                            + ", not one of the supported resultEncodings: "
                            + QueryOptions.getResultEncodingOptions().toString());
                }
            }
        } catch (Exception e) {
